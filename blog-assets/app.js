(function () {
  ...
  function postCard(post, adminMode = false) {
    ...
    return `<article class="post-card">
      <div class="post-meta"><span>${escapeHtml(post.category)}</span><time datetime="${escapeHtml(post.createdAt || post.date)}">${publishedAt}</time><span>预计阅读 ${post.readingMinutes} 分钟</span></div>
  ...
  function enhanceArticle(post) {
    ...
    commentForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (result.published) {
          const refreshed = await interactions.interaction(post.slug);
